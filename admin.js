...
window.initAdminData = function() {
    ...
    onSnapshot(qLive, (snap) => {
        ...
        allOrders.forEach((data) => {
            ...
            if(orderDateStr === selectedDate || data.status === 'New' || data.status === 'Preparing') {
                ...
                if (data.status === 'New' || data.status === 'Preparing') {
                    ...
                    const soundToggle = document.getElementById('soundToggle');
                    if(data.status === 'New' && soundToggle && soundToggle.checked && !isInitialLoad && soundActivated) {
        ...
        const liveCard = document.getElementById('live-order-card');
        if(liveCard) {
...
// ==========================================
// 🪄 TEMPORARY MAGIC FIX SCRIPT (AUTO-TAGGER)
// ==========================================
setTimeout(() => {
    const magicBtn = document.createElement('button');
    magicBtn.innerHTML = "✨ Run Magic Fix";
    magicBtn.style.cssText = "position:fixed; bottom:90px; right:20px; background:#007AFF; color:white; padding:12px 20px; border-radius:50px; border:none; z-index:9999; box-shadow:0 4px 15px rgba(0,122,255,0.4); font-weight:800; cursor:pointer;";
    document.body.appendChild(magicBtn);

    magicBtn.onclick = async () => {
        const confirmFix = confirm("Kya aap saari purani dishes ko FoodVilla (rest_001) ke naam karna chahte hain?");
        if(!confirmFix) return;

        magicBtn.innerHTML = "Fixing...⏳";
        try {
            // Saari dishes ko database se mangwao
            const q = query(collection(db, "menu_items"));
            const snap = await getDocs(q);
            let updatedCount = 0;
            
            // Har dish ko check karo aur update karo
            const updatePromises = [];
            snap.forEach((d) => {
                const data = d.data();
                // Agar restaurantId nahi hai, toh add karo
                if(!data.restaurantId || data.restaurantId === "") {
                    const req = updateDoc(doc(db, "menu_items", d.id), { restaurantId: 'rest_001' });
                    updatePromises.push(req);
                    updatedCount++;
                }
            });

            await Promise.all(updatePromises); // Sab ek sath update hone ka wait karo
            alert(`Jadoo Chal Gaya! 🪄 Total ${updatedCount} purani dishes par FoodVilla ka thappa lag gaya.`);
            magicBtn.remove(); // Kaam hone ke baad button gayab

        } catch(e) {
            alert("Error aagaya bro: " + e.message);
            magicBtn.innerHTML = "✨ Run Magic Fix";
        }
    };
}, 2000); // Page load hone ke 2 second baad button aayega
